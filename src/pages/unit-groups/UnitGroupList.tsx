...
import { UnitGroupService } from '@/services';
...
export const UnitGroupList = () => {
    ...
    const [newUnitRate, setNewUnitRate] = useState<number>(1);
    ...
    const loadGroups = async () => {
        const data = await UnitGroupService.getGroups();
        setGroups(data);
        if (data.length > 0 && !selectedGroupId) setSelectedGroupId(data[0].id);
    };

    const loadUnits = async (groupId: string) => {
        const data = await UnitGroupService.getByGroupId(groupId);
        // Sort: Base unit first, then others
        data.sort((a: Unit, b: Unit) => (a.isBaseUnit === b.isBaseUnit ? 0 : a.isBaseUnit ? -1 : 1));
        setUnits(data);
    };
    ...
    const handleCreateGroup = async () => {
        ...
        try {
            const newGroup = await UnitGroupService.createGroup({ name: newGroupName });
    ...
    const handleDeleteGroup = async (id: string, e: MouseEvent) => {
        ...
        await UnitGroupService.deleteGroup(id);
    ...
    const handleCreateUnit = async () => {
        if (!newUnitName.trim() || !selectedGroupId) return;
        setLoading(true);
        try {
            await UnitGroupService.createUnit({
                groupId: selectedGroupId,
                name: newUnitName,
                isBaseUnit: newUnitIsBase,
                conversionRate: newUnitIsBase ? 1 : newUnitRate
            });
            setNewUnitName('');
    ...
    const saveEditing = async (id: string) => {
        try {
            await UnitGroupService.updateUnit(id, editForm);
            setEditingUnitId(null);
            loadUnits(selectedGroupId);
        } catch (error: any) {
            alert(error.message);
    ...
    const handleDeleteUnit = async (id: string) => {
        ...
        await UnitGroupService.deleteUnit(id);
        loadUnits(selectedGroupId);
    };
                        ...
                        <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
                            ...
                            <div className="p-4 bg-white grid grid-cols-1 sm:grid-cols-4 gap-4 border-b border-slate-100">
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <div
                                        onClick={() => setNewUnitIsBase(!newUnitIsBase)}
                                        className={cn(
                                            "flex items-center gap-2 border rounded-lg px-3 h-10 text-sm cursor-pointer transition-colors",
                                            newUnitIsBase ? "bg-emerald-50 border-emerald-200 text-emerald-700 font-bold" : "bg-white border-slate-200 text-slate-600"
                                        )}
                                    >
                                        <div className={cn("h-4 w-4 rounded border flex items-center justify-center transition-colors", newUnitIsBase ? "bg-emerald-500 border-emerald-500" : "bg-white border-slate-300")}>
                                            {newUnitIsBase && <Check className="h-3 w-3 text-white" />}
                                        </div>
                                        Đơn vị Chuẩn
                                    </div>
                                </div>
                                <div className="space-y-1.5">
                                    ...
                                    <div className="flex gap-2">
                                        <Input
                                            ...
                                            min={1}
                                            value={newUnitRate}
                                            onChange={e => setNewUnitRate(Number(e.target.value))}
                                            disabled={newUnitIsBase}
                                            className="h-10 text-sm border-slate-200 disabled:bg-slate-50 flex-1"
                                        />
                                        <Button onClick={handleCreateUnit} disabled={!newUnitName || loading} className="h-9 bg-indigo-600 text-white hover:bg-indigo-700 px-3 shadow-lg shadow-indigo-200">
                                            <Plus className="h-4 w-4" />
                                        </Button>
                                    </div>
                                </div>
                                                    ...
                                                    <div className="flex items-center justify-end gap-1">
                                                        {isEditing ? (
                                                            ...
                                                            <>
                                                                ...
                                                                <Button size="icon" variant="ghost" className="h-8 w-8 bg-red-600 text-white rounded-md hover:bg-red-700 shadow-sm transition-all" onClick={() => handleDeleteUnit(u.id)}>
                                                                    <Trash2 className="h-3 w-3" />
                                                                </Button>
                                                            </>
                                                        )}
