...
export const OutboundDetailPage = () => {
    ...
    const loadOrder = async (orderId: string) => {
        try {
            ...
            console.error('Failed to load outbound order:', error);
    ...
    if (!order) {
        return <div className="p-8 text-center text-slate-500">Không tìm thấy đơn hàng</div>;
    }
        ...
        <div className="space-y-8">
            <div className="flex items-start justify-between">
                <div className="space-y-1">
                    <h2 className="text-2xl font-bold tracking-tight text-slate-900">Chi tiết Đơn Bán Buôn</h2>
            ...
            <div className="bg-white border rounded-xl shadow-sm overflow-hidden">
                <div className="p-8 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-medium text-slate-600">Ngày xuất kho</label>
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-medium text-slate-600">Trạng thái thanh toán</label>
                            <div className="text-sm font-semibold text-slate-900">{order.paymentStatus}</div>
                        </div>
                    </div>
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {items.map((item, index) => (
                                        <tr key={index} className="hover:bg-slate-50/50 transition-colors">
                                            ...
                                            <td className="p-3 text-right font-semibold text-slate-900">{item.quantity}</td>
                    ...
                    <div className="flex gap-12">
                        <div className="space-y-1">
                            ...
                            <div className="text-xl font-bold text-slate-900">
                                {items.reduce((s, i) => s + i.quantity, 0)} <span className="text-sm font-normal text-muted-foreground">mặt hàng</span>
                            </div>
                        </div>
                        <div className="space-y-1">
